...
One box on Home ("Tell Almanac"). The line goes to Claude with a snapshot (`snapshot.js`: lists, categories, people, routines, this week's tasks, working memory; never the journal) and a tool list (`tools.js`). The model's tool calls are resolved against the state (`resolveCall`, pure, tested by `scripts/test-assistant.mjs`) and applied through ordinary store actions (`applyAction`), with Undo where cheap. No key, no network, or a refused call: the line is filed the plain way (`parseQuickTask` or working memory) so nothing is lost. The key is typed in Settings › Assistant and kept in the device keystore (`anthropicApiKey`); the model (Quick = Haiku 4.5, Careful = Sonnet 5) is a pref. `client.js` is one Messages API round trip with a 30 s timeout.
